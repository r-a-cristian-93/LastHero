...
Action::Action(const Action& a)
	:code(new Code(*a.code))
	,type(new Type(*a.type))
...
Action::Action(Code c, Type t, sf::Vector2i p)
	:code(new Code(c))
	,type(new Type(t))
	,ent_tag(nullptr)
	,ent_name(nullptr)
...
Action::Action(Code c, Type t, sf::Vector2f p, size_t s, size_t f)
	:code(new Code(c))
	,type(new Type(t))
	,ent_tag(nullptr)
	,ent_name(nullptr)
